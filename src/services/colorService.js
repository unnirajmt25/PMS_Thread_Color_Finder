...
export async function addRecord(input) {
  ...
  const withId = { ...record, id: generateId(), updatedAt: today() };
...
export async function updateRecord(id, patch) {
  ...
  const updated = { ...next, updatedAt: today() };
...
export async function renameVendor(oldName, newName) {
  ...
  persist(cache.map((r) => (r.vendor === oldName ? { ...r, vendor: trimmed, updatedAt: today() } : r)));
  const existingFile = vendorFiles.find((f) => f.label === oldName);
  if (existingFile) upsertVendorFileEntry({ ...existingFile, label: trimmed });
}

/**
 * Parses an uploaded vendor thread-chart workbook (.xlsx) entirely in the
 * browser and replaces that vendor's color-mapping records with the freshly
 * parsed ones — re-uploading under the same label is how an admin updates a
 * vendor's chart. Tracked separately in the vendor-file registry so the
 * Admin UI can list uploaded files by label with a last-updated date.
 */
export async function uploadVendorFile(label, file) {
  ...
  if (!file) {
    throw new ValidationError("Choose a .xlsx file to upload.", { file: "Choose a .xlsx file to upload." });
  }
  ...
  const { records: parsed, sheetsParsed, skippedNoThreadNumber, duplicates } = parseWorkbookToRecords(
    workbook,
    trimmedLabel,
    { sheetToJson: XLSX.utils.sheet_to_json, updatedAt }
  );

  if (sheetsParsed === 0) {
    throw new ValidationError(
      `No recognizable thread-chart sheets found in "${file.name}". Expected a header row with columns like "Thread Number" and "PMS Number".`,
  ...
  return { ...entry, sheetsParsed, skippedNoThreadNumber, duplicates };
}
...
export async function importCsv(text) {
  ...
  for (const row of rows) {
    ...
    if (match) {
      next = next.map((r) => (r.id === match.id ? { ...r, ...row, updatedAt: today() } : r));
      updated++;
    } else {
      next = [...next, { ...BLANK_FIELDS, ...row, id: generateId(), updatedAt: today() }];
      added++;
    }
  }
