 ...
 * ColorMapping record shape.
 *
 * Shared by two callers that load workbooks differently:
 ...
 *    folder)
 *  - src/services/vendorFileService.js (browser, `XLSX.read` on an
 *    uploaded File's ArrayBuffer, for the Admin "upload a vendor file"
 *    feature)
 *
 * Keeping the row-shape parsing logic here means both stay in sync
 * instead of drifting into two subtly different implementations.
 */
 ...
 * @param {{ sheetToJson: Function, updatedAt: string }} opts
 ...
 */
export function parseWorkbookToRecords(workbook, vendor, { sheetToJson, updatedAt }) {
  const records = [];
  ...
  for (const sheetName of workbook.SheetNames) {
    ...
    if (iCode === -1) continue; // sheet doesn't match the expected layout
    sheetsParsed++;
